...
class ApiService {
  ...
  Future<List<dynamic>> getFragrancesByBrand(String brand, {int limit = 10}) async {
    ...
    if (response.statusCode != 200) {
      throw Exception('Failed to load fragrances: ${response.statusCode} ${response.body}');
    }
  ...
  Future<List<dynamic>> searchFragrances(String query, {int limit = 20}) async {
    ...
    final url = Uri.parse('$_baseUrl/fragrances?limit=$limit&search=$encodedQuery');
    ...
    if (response.statusCode != 200) {
      throw Exception('Failed to search fragrances: ${response.statusCode} ${response.body}');
    }
